...
content : tuple
    ...
    def match(startcls, subclasses, endcls, reader,
        ...
        assert isinstance(reader,FortranReaderBase),str(reader)
        content = []
...
class Entity_Decl(Base): # R504
    ...
    def match(string, target=False):
        ...
        if newline.startswith('='):
            ...
            assert newline=='',str(newline, string)
        return name, array_spec, char_length, init
    match = staticmethod(match)
    def tostr(self):
        s = str(self.items[0])
...
# start of KGEN
class Defined_Binary_Op(STRINGBase):
    """
...
class Forall_Stmt(StmtBase): # R759
    ...
    def match(string):
        ...
        #if not line.startswith(')'): return # KGEN deletion
        if not line.startswith('('): return # KGEN addition
        i = line.find(')')
        if i==-1: return
        ...
        line = repmap(line[i+1:].lstrip())
        if not line: return
        #return Forall_Header(header), Forall_Assignment_Stmt(line) # KGEN deletion
        return Forall_Header('('+header+')'), Forall_Assignment_Stmt(line) # KGEN addition
    ...
    def match(string):
        for (k,v) in [\
            ...
            try:
                ...
                obj = None
            if obj is not None: return obj
        #return 'UNIT', File_Unit_Number # KGEN deletion
        return 'UNIT', File_Unit_Number(string) # KGEN addition
    match = staticmethod(match)
...
items : (Inquire_Spec_List, Scalar_Int_Variable, Output_Item_List)
    ...
    def match(string):
        ...
        line = string[7:].lstrip()
        if not line.startswith('('): return
        #i = line.find(')') # KGEN addition
        if line.endswith(')'): # KGEN deletion
        #if i!=-1 and i==len(line)-1: # KGEN addition
    ...
    def tostr(self):
        if self.items[0] is None:
            assert None not in self.items[1:],str(self.items)
...
::
    ...
    def tostr(self):
        ...
        raise NotImpletenetedError(str(c))
    ...
    def match(string):
        ...
        if c in ['I','B','O','Z']:
            ...
            if '.' in line:
                ...
                i1 = i1.rstrip()
                i2 = i2.lstrip()
                return c, W(i1), M(i2), None, Int_Literal_Constant # KGEN additon
                #return c, W(i1), M(i2), NoneInt_Literal_Constant # KGEN deletion
            return c,W(line), None, None
        if c=='L':
            line = string[1:].lstrip()
            if not line: return
...
# start of KGEN addition
class Defined_Operator(STRINGBase):
    subclass_names = []
    def match(string):
