...
#                var.set_type(self.typedecl)
        ...
        while content:
            ...
            if isinstance(stmt, Contains):
                ...
                while isinstance (stmt, Comment):
                    stmt = content.pop(0)
                assert isinstance(stmt, self.end_stmt_cls),str(stmt)
        ...
        if not tosubr:
            if isinstance(self, Statement):
                if hasattr(self, 'typedecl') and self.typedecl is not None:
                    assert isinstance(self, Function),str(self.__class__.__name__)
...
class IfThen(BeginStatement):
    ...
    def process_item(self):
        ...
        assert line[0]=='(' and line[-1]==')',str(line)
...
#        return BeginStatement.process_item(self)
    ...
    def tostr(self):
        assert len(self.content)==1,str(self.content)
