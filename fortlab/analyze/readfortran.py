...
class Line(object):
    ...
    def __init__(self, line, linenospan, label, name, reader):
        ...
        assert self.line, str(line, linenospan, label)
        self.span = linenospan
        assert label is None or isinstance(label,int),str(label)
        assert name is None or isinstance(name,str) and name!='',str(name)
        self.label = label
        self.name = name
        self.reader = reader
        self.strline = None
...
#            for d in self.include_dirs: # KGEN deletion
    ...
    def next(self, ignore_comments = False):
        ...
        try:
            ...
            logger.critical('STOPPED READING: %s' % str(err)) # KGEN addition
    ...
    def handle_cf2py_start(self, line):
        ...
        if m:
            ...
            self.f2py_comment_lines.append(self.linecount)
            assert len(newline)==len(line),str(newlinel,line)
            return newline
        return line
...
cf2py call me ! hey
    ...
    assert reader.mode=='fix77', str(reader.mode)
    for item in reader:
        print(item)
...
"""
    ...
    assert reader.mode=='pyf', str(reader.mode)
    for item in reader:
        print(item)
...
"""
    ...
    assert reader.mode=='fix90', str(reader.mode)
    for item in reader:
        print(item)
