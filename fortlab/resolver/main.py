...
class FortranNameResolver(App):
    ...
    def __init__(self, mgr):
        ...
        self.add_argument("--save", metavar="analysis", help="save analysis")

        ###############################################################
        # Add common options
        ###############################################################
        self.add_argument("-i", "--include-ini", dest="include_ini", action='store', type=str, default=None, help="information used for analysis")
        self.add_argument("-e", "--exclude-ini", dest="exclude_ini", action='store', type=str, default=None, help="information excluded for analysis")
        self.add_argument("-I", dest="include", action='append', type=str, default=None, help="include path information used for analysis")
        self.add_argument("-D", dest="macro", action='append', type=str, default=None, help="macro information used for analysis")
        self.add_argument("--outdir", dest="outdir", action='store', type=str, default=None, help="path to create outputs")
        self.add_argument("--source", dest="source", action='append', type=str, default=None, help="Setting source file related properties")
        self.add_argument("--intrinsic", dest="intrinsic", action='append', type=str, default=None, help="Specifying resolution for intrinsic procedures during searching")
        self.add_argument("--machinefile", dest="machinefile", action='store', type=str, default=None, help="Specifying machinefile")
        ...
        self.add_argument("--invocation", dest="invocation", action='append', type=str, default=None, help="(process, thread, invocation) pairs of kernel for data collection")
        self.add_argument("--data", dest="data", action='append', type=str, default=None, help="Control state data generation")
        self.add_argument("--openmp", dest="openmp", action='append', type=str, default=None, help="Specifying OpenMP options")
        self.add_argument("--mpi", dest="mpi", action='append', type=str, default=None, help="MPI information for data collection")
        self.add_argument("--timing", dest="timing", action='store', type=str, default=None, help="Timing measurement information")
        self.add_argument("--prerun", dest="prerun", action='append', type=str, default=None, help="prerun commands")
        self.add_argument("--rebuild", dest="rebuild", action='append', type=str, default=None, help="rebuild controls")
        self.add_argument("--state-switch", dest="state_switch", action='append', type=str, default=None, help="Specifying how to switch orignal sources with instrumented ones.")
        self.add_argument("--kernel-option", dest="kernel_option", action='append', type=str, default=None, help="Specifying kernel compiler and linker options")
        self.add_argument("--check", dest="check", action='append', type=str, default=None, help="Kernel correctness check information")
        self.add_argument("--verbose", dest="verbose_level", action='store', type='int', default=None, help="Set the verbose level for verification output")
        self.add_argument("--add-mpi-frame", dest="add_mpi_frame", action='store', type=str, default=None, help="Add MPI frame codes in kernel_driver")
        ...
        self.add_argument("--noreuse-rawdata", dest="reuse_rawdata", action='store_false', default=True, help="Control raw data generation for modeling.")
        self.add_argument("--repr-etime", dest="repr_etime", action='append', type=str, default=None, help="Specifying elapsedtime representativeness feature flags")
        self.add_argument("--repr-papi", dest="repr_papi", action='append', type=str, default=None, help="Specifying papi counter representativeness feature flags")
        self.add_argument("--repr-code", dest="repr_code", action='append', type=str, default=None, help="Specifying code coverage representativeness feature flags")
...
#
            ...
            if mpifpath:
                try:
                    ...
                    for config_key, name in config_name_mapping:
                        if not self.config['mpi'].has_key(config_key) or self.config['mpi'][config_key] is None:
    ...
    def _process_default_flags(self, opts):
        ...
        if opts.include_ini:
            self.config['include']['opt'] = opts.include_ini

        if opts.exclude_ini:
            process_exclude_option(opts.exclude_ini, self.config['exclude'])
        ...
        if opts.outdir:
            self.config['path']['outdir'] = opts.outdir
    ...
    def _process_extract_flags(self, opts):
        ...
        if opts.cmd_clean:
            self.config['cmd_clean']['cmds'] = dequote(opts.cmd_clean)


        if opts.cmd_build:
            self.config['cmd_build']['cmds'] = dequote(opts.cmd_build)

        if opts.cmd_run:
            self.config['cmd_run']['cmds'] = dequote(opts.cmd_run)
        ...
        if opts.verbose_level:
            self.config['verify']['verboselevel'] = str(opts.verbose_level)
        ...
        if opts.add_cache_pollution:
            if opts.add_cache_pollution.isdigit():
                ...
                self.config['add_cache_pollution']['size'] = int(opts.add_cache_pollution)
            else:
    ...
    def _process_repr_flags(self, opts):
        ...
        self.config['model']['reuse_rawdata'] = opts.reuse_rawdata
    ...
    #                incattrs[lsection][option] = subflags
        ...
        for path in self.config['include']['path']:
            newpath.add(path)
            for p1, p2 in self.config['source']['alias'].iteritems():
                if path.startswith(p1):
                    newpath.add(p2+path[len(p1):])
                elif path.startswith(p2):
                    newpath.add(p1+path[len(p2):])
        ...
        newfile =  OrderedDict()
        for path, value in self.config['include']['file'].iteritems():
            newfile[path] = value
            for p1, p2 in self.config['source']['alias'].iteritems():
                if path.startswith(p1):
                    newpath = p2+path[len(p1):]
