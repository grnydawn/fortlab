...
from fortlab.kggenfile import genkobj, gensobj, KERNEL_ID_0, event_register, create_rootnode, create_programnode, init_plugins, append_program_in_root, set_indent, plugin_config
from fortlab.kgutils import ProgramException, remove_multiblanklines, run_shcmd, tounicode
from fortlab.resolver.kgparse import KGGenType
from fortlab.kgextra import kgen_utils_file_head, kgen_utils_file_checksubr, kgen_get_newunit, kgen_error_stop, kgen_utils_file_tostr, kgen_utils_array_sumcheck, kgen_rankthread
...
class FortranKernelGenerator(App):
    ...
    def perform(self, args):
        ...
        if args.model:
            if not os.path.exists(args.model['_']):
                kgutils.UserException('Model file, %s, does not exist.' % args.model['_'])
            self.read_model(args.model['_'], self.config)
        else:
            self.config['invocation']['triples'].append((('0', '0'), ('0', '0'), ('1', '1')))
        plugins = (('ext.gencore', gencore_plugindir),
         (
        ...
        for filepath, (srcobj, mods_used, units_used) in self.config['srcfiles'].items():
            if hasattr(srcobj.tree, 'geninfo') and KGGenType.has_state(srcobj.tree.geninfo):
                ...
                self.config['used_srcfiles'][filepath] = (kfile, sfile, mods_used, units_used)
        ...
        for kfile, sfile, filepath in self.genfiles:
            ...
            if klines is not None:
                ...
                with io.open(os.path.join(kernel_realpath, filename), 'w', encoding=enc) as (fd):
            ...
            if sfile.kgen_stmt.used4genstate:
                ...
                if slines is not None:
                    ...
                    with io.open(os.path.join(state_realpath, filename), 'w', encoding=enc) as (fd):
                        fd.write(tounicode(slines))

        with io.open(os.path.join(kernel_realpath, '%s.f90' % self.config['kernel_driver']['name']), 'w', encoding=enc) as (fd):
            set_indent('')
    ...
    def generate_kernel_makefile(self, kernelpath, enc):
        openmp_flags = [
         '-fopenmp', '-qopenmp', '-mp', '-xopenmp', '-qsmp=omp']
        callsite_base = os.path.basename(self.config['callsite']['filepath'])
        driver_base = '%s.f90' % self.config['kernel_driver']['name']
        dep_base_srcfiles = [ os.path.basename(filepath) for filepath, srclist in self.config['used_srcfiles'].items() ]
        ...
        for filepath, (kfile, sfile, mods_used, units_used) in self.config['used_srcfiles'].items():
            dep = [
             self.obj(KGUTIL)]
            for mod in mods_used:
                if mod.reader.id != filepath and self.obj(os.path.basename(mod.reader.id)) not in dep:
                    dep.append(self.obj(os.path.basename(mod.reader.id)))

            for unit in units_used:
                if unit.item.reader.id != filepath and self.obj(os.path.basename(unit.item.reader.id)) not in dep:
        ...
        for path, kfile in self.config['include']['file'].items():
            ...
            if 'compiler_options' in self.config['include']['compiler']:
                opts = opts + ' ' + self.config['include']['compiler']['compiler_options']
        ...
        if 'import' in self.config['include']:
            for path, import_type in self.config['include']['import'].items():
                if import_type == 'library' or import_type == 'shared_library':
                    ...
                    shutil.copy(path, '%s/%s' % (self.config['path']['outdir'], self.config['path']['kernel']))
        ...
        with io.open(os.path.join(kernelpath, 'Makefile'), 'w', encoding=enc) as (f):
            ...
            if self.config['kernel_option']['FC']:
                ...
                if self.config['kernel_option']['FC_FLAGS']:
                    self.write(f, 'FC_FLAGS_SET_0 := %s' % self.config['kernel_option']['FC_FLAGS'])
                else:
                        ...
                        if L > 1:
                            ...
                            for opt in opt_list:
                                ...
                                if opt in self.config['kernel_option']['compiler']['remove']:
                                    pass
                                elif '%s+' % opt in self.config['kernel_option']['compiler']['remove']:
                                    skip_next = True
            ...
            prerun_build_str = ''
            if self.config['prerun']['kernel_build'] or self.config['prerun']['build'] or self.config['prerun']['kernel_run'] or self.config['prerun']['run']:
                self.write(f, '# Comment-out below if your system environment is different from one that this kernel is extracted from.')
                self.write(f, '# NOTE: the environment is valid only if you are using this kernel on the same computing system that the kernel was extracted from.')
            if self.config['prerun']['kernel_build']:
                self.write(f, 'PRERUN_BUILD := %s; ' % self.config['prerun']['kernel_build'])
            ...
            if self.config['prerun']['kernel_run']:
                self.write(f, 'PRERUN_RUN := %s; ' % self.config['prerun']['kernel_run'])
                ...
                if self.config['model']['types']['papi']['enabled']:
                    self.write(f, 'PAPI_EVENT := %s' % self.config['model']['types']['papi']['event'])
                    self.write(f, '')
                ...
                if self.config['add_mpi_frame']['enabled']:
                    self.write(f, '%s%s -np %s ./kernel.exe' % (prerun_run_str, self.config['add_mpi_frame']['mpirun'], self.config['add_mpi_frame']['np']), t=True)
                else:
                ...
                if self.config['model']['types']['papi']['enabled']:
                    ...
                    if self.config['model']['types']['papi']['static'] is not None:
                        link_flags += ' %s' % self.config['model']['types']['papi']['static']
                    if self.config['model']['types']['papi']['dynamic'] is not None:
                        ddir, dlib = os.path.split(self.config['model']['types']['papi']['dynamic'])
                ...
                for dep_base in dep_bases:
                    ...
                    if self.config['model']['types']['papi']['enabled'] and self.config['model']['types']['papi']['header'] is not None and dep_base in [callsite_base, driver_base]:
                        self.write(f, 'ifeq (${MAKECMDGOALS}, papi)')
                        papi_flags_str = ' -DKGEN_PAPI -I%s' % os.path.split(self.config['model']['types']['papi']['header'])[0]
                        self.write(f, '%s %s %s $< | sed "s/KGENPAPIEVENT/${PAPI_EVENT}/g" > tmp.$<' % (self.config['bin']['pp'], self.config['bin']['cpp_flags'], papi_flags_str), t=True)
                        self.write(f, 'else')
                        self.write(f, '%s %s $< tmp.$<' % (self.config['bin']['pp'], self.config['bin']['cpp_flags']), t=True)
                        self.write(f, 'endif')
    ...
    def generate_state_makefile(self, statepath, enc):
        ...
        org_files = [ filepath for filepath, (kfile, sfile, mods_used, units_used) in self.config['used_srcfiles'].items() if sfile.kgen_stmt.used4genstate ]
        ...
        with io.open(os.path.join(statepath, 'Makefile'), 'w', encoding=enc) as (f):
            ...
            if self.config['state_switch']['type'] == 'replace':
                ...
                for org_file in org_files:
                    ...
                    self.write(f, 'cp -f %s/%s/%s %s' % (self.config['path']['outdir'], self.config['path']['state'],
                     basename, self.config['state_switch']['directory']), t=True)
